 ...
 * Date:        6/1/2017
 * Description: Handles all logic and calculations of a Infix Expression
 */
class InfixEval {
     ...
     * @throws InvalidCharacterException is thrown when characters other than 0-9 and +,-,*,/,(,), are used
     */
    String evaluate(String equation) throws DivideByZero, InvalidCharacterException, EmptyStackException {
        ...
        for (String token : tokens) {
            ...
            // Checks if token is an operator
                ...
                while (!operator.empty() && precedence(operator.peek()) >= precedence(token)) { //does not recheck precedence with new operators after first execution
                    operand.push(calculation(operand.pop(), operator.pop(), operand.pop()));
                }
                // Adds to operator stack
                operator.push(token);
